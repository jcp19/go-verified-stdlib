...
- Verification *errors* are a different problem from slowness. When Gobra reports an error whose cause is not obvious, the skill `gobra-locate-failure` narrows the failing obligation down to the statement that destroys the fact and the fact that is missing, instead of guessing at extra annotations. It also covers the check for vacuous proofs (`assert false` / `refute false`), which is worth running over any member you have just finished proving.
